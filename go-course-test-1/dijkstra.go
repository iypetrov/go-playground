...
func (g Graph) Path(start, target int) (path []int, cost float64, err error) {
	if len(g) == 0 {
	...
	explored := make(map[int]bool) // set of nodes we already explored
	frontier := NewQueue()         // queue of the nodes to explore
	previous := make(map[int]int)  // previously visited node
